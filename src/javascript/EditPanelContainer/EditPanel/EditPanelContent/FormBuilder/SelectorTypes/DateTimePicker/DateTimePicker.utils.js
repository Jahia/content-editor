...
export function fillDisabledDaysFromJCRConstraints(field, datetime) {
    if (field.formDefinition.valueConstraints && field.formDefinition.valueConstraints.length > 0) {
        const disableDays = [];
        const {lowerBoundary, disableLowerBoundary, upperBoundary, disableUpperBoundary} = extractRangeConstraints(field.formDefinition.valueConstraints[0].value.string);
